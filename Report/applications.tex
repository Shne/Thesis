...
A string $S$ of $n$ characters is transformed by the Burrows-Wheeler transformation~\citeA{BWToriginalArticle} by forming $n$ cyclic shifts of $S$. 
...
Additional uses for storing sequences, according to G.Navarro~\citeA[Section 5]{Navjda13}, include storing and querying graphs.
...
The wavelet tree can be used to support efficient range quantile queries on a sequence \textit{S} of \textit{n} numbers in $O(\log(\sigma))$ time if $rank_b$ is supported in $O(1)$ time~\citeA[Section 3]{RangeQuantileQueries}. 
