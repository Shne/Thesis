...
RLE is still very useful despite of this limitation because you usually want to compress very large data and if that data uses an alphabet that is small enough then, as previously stated RLE can achieve compression close to the zero order entropy when working with binary alphabets.
...
Figure~\ref{fig:BwtRleWaveletTree} depicts a RLE Wavelet tree on BWT($abcbadbd$). We can see, as indicated by the numbers in bold that fewer Run-length encoded values needs to be stored than for the non-Burrows-Wheeler transformed case as shown in figure~\ref{fig:RleWaveletTree}.
\figureBegin
...
One might wonder how \textproc{Rank} and \textproc{Select} queries can be useful when the string is Burrows-Wheeler transformed and therefore permuted resulting in the queries values that is essentially unrelated to the original string $S$. 
