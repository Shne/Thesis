...
There are two versions: empirical zero-order entropy $H_0$ and empirical $k$th-order entropy $H_k$, and they are defined in Definition~\ref{def:emp0entropy} and \ref{def:empkentropy}. $H_k$ takes into account a context of size \textit{k} of the symbol appearances, i.e. the suffixes of length $k$ of each symbol appearance in the string, while $H_0$ does not and treats symbols independently instead. 
