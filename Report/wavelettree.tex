...
The problem then for a long time was that there existed no good way to achieve compression within the $H_0$ lower bound or at least it was a problem before the wavelet tree was invented. \textbf{(todo: describe why it was a problem)} 
...
\textit{Zero-order entropy} can also be achieved by run-length encoding the bitmaps in the wavelet tree, which is the approach that is used when compressing the bwt using the wavelet tree \citep[Introduction (\textbf{B})]{waveletTreeEntropy}.
Using a combination of run-length encoding and bwt on the wavelet tree it is possible to achieve a lower bound within \textit{k-th order entropy}
