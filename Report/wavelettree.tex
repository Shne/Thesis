...
Then by looking at the bitmap of \textit{aaaaacccaaaaabbbaa} which is \textit{000001110000000000} we can encode it into \textit{{5 3 10}}.
Figure~\ref{ig:RleWaveletTree} shows an example of a wavelet tree with run-length encoded bitmaps.
...
I do consider how a computer saves numbers then the reduction above is not that great.
If we assume that each number above is represented as an integer, then the run-length encoded bitmap uses more space than just storing the original bitmap. 
...
\Tree
%root
[.abcbadbd\\212111 !\qsetw{5cm} 
	%left child
	[.abbab\\1211  !\qsetw{5cm}
		%left -> left,right child 
...
\Tree
%root
[.aabbbcdd\\\textbf{53} !\qsetw{5cm} 
	%left child
	[.aabbb\\\textbf{23}  !\qsetw{5cm}
		%left -> left,right child 
		[.aa\\2 !\qsetw{5cm} ] [.bbb\\3 !\qsetw{5cm} ]] 
	%right child
	[.cdd\\12 !\qsetw{5cm} 
