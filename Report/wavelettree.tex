...
\textproc{BitmapConstruction} works by first splitting the alphabet in two pieces of equal size: one for the left child and one for the right child.
...
\end{algorithmic}
\end{algorithm}
A cornerstone of the \textproc{Rank} query on the Wavelet Tree is calculating the \textproc{BinaryRank} of a character on a bitmap at a node.
