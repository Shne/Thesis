...
At the root node is stored a bitmap the length of the input string, and each bit in this bitmap represents which half of the alphabet the character at the same position resides in.
...
The input string alphabet can be stored in the tree.
...
At each level in the tree at most \textit{n} bits are stored the bitmaps in total.
If some characters of the alphabet don't occur in the input string, the sub trees of the Wavelet tree corresponding to those missing characters may be omitted depending on implementation.
