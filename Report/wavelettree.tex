...
Before we can continue down into the child node, we must know what position in the left (or right) substring $S_{left}$ (or $S_{right}$) the character has been mapped to.
...
We continue this traversal until we reach a leaf node that will then correspond to the character at the original position $p$ given to the query.
...
\State Occurrence $\gets$ \Call {BinaryRank} {CharBit, $p$}
\If{CharBit $= 1$}
	\State Rank $\gets$ RightChildNode.\Call {Rank} {$c$, Occurrence}
\Else
	\State Rank $\gets$ LeftChildNode.\Call {Rank} {$c$, Occurrence}
\EndIf
\State \Return Rank
\EndFunction
\end{algorithmic}
...
In Algorithm~\ref{alg:select} we call the function performing this traversal \textproc{GetLeaf}.
