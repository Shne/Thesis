...
Then we do a bitwise \texttt{AND} operation on this bitmask and the machine word containing the bit corresponding to $Position$, and call \texttt{popcountl} on the result. 
Using \texttt{popcount} \textproc{BinaryRank} becomes a $O(N/wordsize) = O(N)$ operation. So in theory still O(N) since $wordsize$ is a constant factor, but it has a large practical effect on performance.
...
We split Select into two functions; \textproc{Select} which is called on the leaf and \textproc{SelectRec} which is called on the parent of the leaf and continues recursively until the root is reached. 
...
This takes O(N) time making \textproc{BinarySelect} a huge bottleneck for \textproc{Select} since is it called for each level in the Wavelet Tree. 
