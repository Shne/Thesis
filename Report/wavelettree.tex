...
\EndIf
\State $S \lbrace l..r \rbrace$\textit{-length} $ \gets r-1$
\State \textit{1}$sInRange \gets rank_b(S \lbrace l..r \rbrace) = rank_b(r) - rank_b(l-1)$
\State $\textit{0}sInRange \gets S \lbrace l..r \rbrace$\textit{-length }$- \textit{1}sInRange$
\If{$\textit{0}sInRange \leq k$}
	\State $l = (S \lbrace l..r \rbrace$\textit{-length} $ - rank_b(l-1))+1$
	\State $r = S \lbrace l..r \rbrace$\textit{-length} $ - rank_b(r)$
	\State \Return $LeftNode.RangeQuantileQuery(k,l,r)$
\Else
	\State $k = k - \textit{0}sInRange)$
	\State $l = rank_b(l-1)+1$
	\State $r = rank_b(r)$
...
We go to the left if there are more than \textit{k} copies of 0s $b[l..r]$ and set $l = ($number of 0s in $b[1..(l-1)])+1$ and $r=($number of 0s in $b[1..r])$. 
We go to the right if there are less than \textit{k} copies of 0s $b[l..r]$ and subtract the number of 0s in $b[l..r]$ from \textit{k}, set $l = ($number of 1s in $b[1..(l-1)])+1$ and set $r=($number of 1s in $b[1..r])$. This procedure continues recursively until it hits a leaf and then returns the number stored in the leaf which corresponds to the k-th smallest number in the given range of \textit{S}.
Algorithm~\ref{alg:RangeQuantileQuery} describes the pseudo-codo for a range quantile query.
$rank_b$ is a binary rank query which returns the number of 1s in the bitmap of $S$ in each node generated by the wavelet tree. 
This means that $rank_b(r)$ for instance returns the number of 1s in the bitmap until position $r$.
...
$k=5$ means that we are looking for the 5th smallest number within $S[l..r]$ which is \textit{7}.
