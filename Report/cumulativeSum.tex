...
UnalignedNaive spends 3.6 milliseconds on 1000 queries, where CumulativeSum spends 3.0 milliseconds, an improvement of 15.87\%.
...
In Figure~\ref{fig:CumulativeSumRankTLBMiss} we see that the CumulativeSum tree has fewer Translation Lookaside Buffer Misses, although we can see from the errorbars that they their standard deviations overlap, so perhaps there is less of an improvement here than it might look at first glance.
...
For level 2 cache, the amount of cache misses of CumulativeSum is even within the standard deviation of the cache misses for UnalignedNaive.
We also see a slightly worse level 3 cache performance in Figure~\ref{fig:CumulativeSumRankL3CM}.
\textbf{[TODO: speculate as to why]}
...
We believe the explanation for the decrease in level 2 cache hits lies in the reduction of level 1 cache misses, like results from previous experiments.
...
The reduction of level 1 cache misses is on average $\num{134783.8}$ and the reduction in level 2 cache hits is on average $\num{136605.6}$ making them very close.
