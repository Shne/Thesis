...
This is also a conclusion that Piantadosi~\citeA[abstract]{ZipfsLawOnText} arrives at.
...
We tried using the word frequencies in the NGSL~\citeA{NGSL} wordlist which contain the 31,241 most used words in the English language and the frequency with which they appear, to estimate \textit{s}.
...
Estimating \textit{s} using a sunsequence of words from NGSL gave us a value close to 1 and it only grew closer to 1 the more words we used from the list.
