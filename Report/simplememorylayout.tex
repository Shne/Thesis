...
In this experiment we aim to show that the controlled memory layout is important in order to reduce cache misses, and that even though the skew might reduce branch mispredictions further, the increase in tree depth makes it not worth it for the algorithm without controlled memory layout.
