...
We now analyse the theoretical worst case running time of constructing and querying a balanced wavelet tree vs. a skewed wavelet tree.
Constructing a balanced wavelet tree takes  $O(n \cdot \log \sigma)$ time because the hight of the tree is $\lceil \log \sigma \rceil$ and there are $n$ elements in each level.
...
The tree then has a hight of $O(\sigma)$, which is larger than the height $\lceil \log \sigma \rceil$ of the balanced tree.
