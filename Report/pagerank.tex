...
While we expect that this will save some expensive TLB misses it will also require some additional computations to find the correct block to access in a query.
For the wavelet trees using concatenated bitmaps, these computations are needed regardless of block alignment, as the blocks are already unlikely to be aligned with the individual bitmaps, and so we believe it can only be an improvement to page-align the blocks in this case. \textbf{[TODO: should we test this anyway?]}
...
While much of the increased running time of Rank queries on the PreallocatedPrecomputed wavelet tree can likely be explained by the increased amount of instructions needed to calculate the rank of the first part inside the first block of each bitmap when not page aligned, as well as the ineffectiveness of utilizing the precomputed rank values for small bitmaps.
However, part of the increase is also due to the increased number of branches in the code.
We come to this conclusion by examining Figure~\ref{fig:PrecomputedRankBlockSize_Rank_BranchMiss} and \ref{fig:PrecomputedRankBlockSize_Rank_BranchMissRate} where we can see that 
