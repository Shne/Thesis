...
While this could potentially save us some expensive TLB misses it will also require more computations to find the correct block to access in a query, in the data structure not using concatenated bitmaps.
For the data structure using concatenated bitmaps, these computations are needed regardless of block alignment, and so we believe it can only be an improvement to page-align the blocks in this case. \textbf{[TODO: should we test this anyway?]}
Whether this is a worthwhile modification when not using concatenated bitmaps will be tested in section~\ref{sec:queryRunTimePrecomputedBlockSizes}.
...
In Figure~\ref{fig:PrecomputedRankBlockSize_Rank_WallTime} we have plotted the wall time in $\mu s$ for the algorithms using precomputed rank values.
Not included in the graph is the wall time for the non-precomputed case, NaiveInteger, that is about a factor 375 of the fastest running time in this graph, UnalignedNaivePrecomputed at block size = 0.5 page.

We see that PreallocatedPrecomputed is consistently slower than the other two, as expected, taking about 90\% more time than UnalignedNaivePrecomputed with block size = 0.5 page where Unaligned is fastest, and 63\% more with block size = 1 page where Preallocated is fastest.

While much of the increased running time of Rank queries on the PreallocatedPrecomputed wavelet tree can likely be explained by the increased amount of instructions needed, as can be seen by comparing the pseudocode in Algorithm~\ref{alg:TODO} and Algorithm~\ref{alg:TODO} \textbf{[TODO]}, part of the increase is also due to the increased number of branches in the code.
We come to this conclusion by examining Figure~\ref{fig:PrecomputedRankBlockSize_Rank_BranchMiss} and \ref{fig:PrecomputedRankBlockSize_Rank_BranchMissRate} 
...
\figureBegin
\caption{Difference in Memory Usage of wavelet trees with precomputed ranks of varying block size. Limited to 2 times page size because of use of \texttt{ushort}}
