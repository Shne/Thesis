...
Assuming the precomputed values are then stored as 16-bit unsigned short integers it will only consume an extra 16 bits or 2 bytes per block and there there are $\frac{\mathit{BitmapSize}}{\mathit{BlockSize}}$ of these blocks when the bitmaps are concatenated.
