...
This also matches what we can see in Figure~\ref{fig:PrecomputedRankBlockSize_Select_L2CacheMiss}.
...
\figureBegin
\caption{Branch Mispredictions for Select queries on wavelet trees with precomputed ranks of varying block sizes. Notice the y-axis not starting at 0.}
...
We have used Massif, the heap profiler included in the Valgrind Suite to record the heap memory usage of our programs.
We have not simply used the snapshots produced automatically by Massif, but rather used the Client Request mechanism of valgrind to send a 'snapshot' command to the valgrind gdbserver, telling it to take a snapshot right after we've finished building the tree.
...
\label{fig:PrecomputedRankBlockSize_Build_Memory}
