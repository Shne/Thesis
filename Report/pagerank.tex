...
Not included in the graph is the running time for the non-precomputed case NaiveInteger that is about a factor 375 of the fastest running time in this graph.

We see that PreallocatedPrecomputed is consistently worse than the other two, as expected, taking about 90\% more time than UnalignedNaivePrecomputed with block size = 0.5 page where Unaligned is fastest and 63\% more with block size = 1 page where Preallocated is fastest.
...
Whether it actually does reduce TLB misses can be seen in Figure~\ref{fig:PrecomputedRankBlockSize_Rank_TLB} where we see that it does reduce TLB misses, but only by about 4,45\% at block size = $\nicefrac{1}{2}$ page size which is even within the average relative standard deviation of both NaivePrecomputed and UnalignedNaivePrecomputed.
