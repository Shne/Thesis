...
The bitmaps are allocated as as one giant bitmap the size of the maximum possible size required to store all the bitmaps for all the nodes.
...
More precisely, the algorithm might at most load \[2 \cdot \left( \left\lceil\frac{blockSize}{pageSize}\right\rceil +1 \right)\] pages to do the popcount binary rank computation at the beginning and end of each node.
...
The rank query is a computation of worst-case cost $\frac{blockSize}{2}$ because it will at most popcount half of the block, because the precomputed rank value is utilized when advantageous.
...
Assuming the precomputed values are then stored as 16-bit unsigned short integers it will only consume an extra 16 bits or 2 bytes per block and there there are $\frac{BitmapSize}{BlockSize}$ of these blocks when the bitmaps are concatenated.
