...
Whether or not using precomputed values in blocks is an improvement in running time or not, depends on which block size is used.
...
Later, in Section~\ref{} \textbf{[TODO]}, we show using a fixed block size $b$ that it does indeed depend on $n$ and the practically optimal value of $b$ is below the theoretically optimal size of $b$ for the root bitmap.
