...
So, in the worst case, when the sought-after occurrence is in the first block of the bitmap, $\frac{blockSize}{2}$ work is wasted, yet when the occurrence is elsewhere, $blockSize$ work is saved.
This means that, for worst-case, the boundary between where using the rank query becomes a gain or a loss in performance is where the ratio between times the occurrence is to be found in and outside the first block of the bitmap is $\nicefrac{2}{3}$.
That is, when the occurrence is to be found outside the first block of the bitmap less than two-thirds of the time, using the rank query first to see if that block should be select queried is a gain in performance, when considering the worst-case scenario.
