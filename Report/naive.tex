...
binary rank was implemented as a simple linear scan of the bitmap.
Select queries were implemented as described in Section~\ref{sec:selectDescription} with binary select implemented as a simple linear scan of the bitmap.
...
When using \texttt{popcount}, \textproc{BinaryRank} remains in theory an $\smash O(\frac{N}{\mathit{wordsize}}) = O(N)$ operation, as \textit{wordsize} is a constant factor, but it has a large practical effect on performance as can be seen in Section~\ref{sec:experimentPopcountRankSelect}.
...
We define the input occurrence parameter as \textit{io}, the number of words iterated through so far as \textit{w}, the sum so far as \textit{sum}, and the wordsize as \textit{ws}. 
The occurrence argument for that last simple binary select is then the $\mathit{io} - \mathit{sum}$ and the output position is $w \times \mathit{ws} + \textproc{BinaryRank}(\mathit{bitmapwords}[\mathit{ws}+1], io - \mathit{sum})$ \footnote{Using simple binary rank without \texttt{popcount}}.
...
The actual running time should then be $a \cdot n \log\sigma + b \sigma$.
Since $n$ in our previous experiment is somewhat large ($10^8$), it might be the dominating factor in the running time.
So to show whether the added $b \sigma$ term can explain the running time, we redid the experiment with a reduced length input string $n = 10^2$ and plotted it in Figure~\ref{fig:naiveIntegerAlphabetSize_WallTime_plusSigma} divided by $\sigma + \log \sigma$ to see whether it would converge on some constant as $\sigma \rightarrow \infty$.
...
Given that the branch misprediction penalty on the Ivy Bridge architecture (on which this experiment was run) is about ``15 cycles or more''\footnote{Section 3.7 in \url{http://www.agner.org/optimize/microarchitecture.pdf}}, we can calculate an estimate of how many cpu cycles the branch misprediction reduction has saved us.
...
\begin{figure}\tiny
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\caption{Wall Time divided by $\sigma + \log \sigma$}
