...
Because the plot in Figure~\ref{fig:naiveIntegerAlphabetSize_WallTime} is clearly not a straight line, we can conclude that the running time of our implementation is not logarithmic on the size of the alphabet size.
...
In Figure~\ref{fig:naiveIntegerAlphabetSize_BM} we see the amount of branch mispredictions for each alphabet size, again with a logarithmic x axis to better compare with Figure~\ref{fig:naiveIntegerAlphabetSize_WallTime}.
We see that the branch mispredictions grow at a rate similar to the wall time.
In Figure~\ref{fig:naiveIntegerAlphabetSize_TLB_CM} we see cache misses and translation lookaside buffer misses plotted in a similar fashion and we can see that they do not display a growth similar to the wall time and branch mispredictions.
This leads us to conclude that branch mispredictions are likely the reason our algorithm doesn't perform as it theoretically should.
...
\begin{figure}
	...
	\begin{subfigure}{\textwidth}
		\input{naiveIntegerAlphabetSize_BM}
		\caption{Branch Mispredictions}
		\label{fig:naiveIntegerAlphabetSize_BM}
	\end{subfigure}
\end{figure}

\begin{figure}
	\input{naiveIntegerAlphabetSize_TLB_CM}
	\caption{Tree Construction: Translation Lookaside Buffer Misses, and Level 1-3 Cache Misses over Alphabet Size}
	\label{fig:naiveIntegerAlphabetSize_TLB_CM}
\end{figure}
