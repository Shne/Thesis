...
Our use of \texttt{popcount} to improve binary rank and select queries was inspired by~ González et al.~\cite{Gonzalez05practicalimplementation} who used it to improving binary rank and binary select for bit arrays.
...
As also noted in~\citep{Navjda13}, we do not need to count the number of 0s, although required by the algorithm, as we can simply take the number of bits in the string and subtract the number of 1s to calculate the number of 0s.
...
This means our implementation of the construction of a Wavelet Tree is not keeping within its theoretical running time for higher alphabet sizes.
...
We also attempted using an alphabet size of 32, but our machines did not have enough memory.
