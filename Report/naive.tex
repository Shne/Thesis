...
<<<<<<< Updated upstream
This section deals with the simple, straightforward, naïve implementation based on ~the description by Navarro~\citep[Section 2]{Navjda13}, before any smart ideas and optimizations were introduced.
...
To improve \textproc{BinaryRank}, we used the intrinsic cpu instruction \texttt{popcount} which will count the number of 1s in the binary representation of the number we pass to it.
...
As also noted in~\citep{Navjda13}, we do not need to count the number of 0s, although required by the algorithm, as we can simply take the number of bits in the bitmap and subtract the number of 1s to calculate the number of 0s.
...
Zachery B. Simpson~\citep{bookWordStatistics} has found that no book occurring in the Gutenberg Project uses more than 43113 distinct words.
