...
Modern programs are however typically full of branch instructions.
...
When the the is correctly predicted there is still a problem.
The address to go to for some conditional branches are computed values. Since computation takes place after fetching the address is unknown and the prediction becomes useless. 
A way to fix this problem is to store the address branched to last time for the particular branch, in the history table.
The previous address can then be used to branch to.


\subsection{Translation Lookaside Buffer misses}
Modern computers uses virtual memory to to address the problems of sharing limited memory between processes and/or users.
Virtual memory hides the presence of physical memory and instead presents an abstract view of main memory.
In particular, it conceals the fact that physical memory is not allocated to a program as a single continuous region while also concealing the actual size of the main memory.
Virtual memory creates the illusion that the available memory for a given process is larger than what is physically available.
This illusion is accomplished by subdividing
